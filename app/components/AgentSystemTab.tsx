...
const DATA: Record<string, NodeDetail> = {
  ...
  human:    { name: "Human review gate",           phase: "Phase 1",                                     role: "Quality control before anything reaches the internet. This is a person.",                                                                              owns: "Final approval for Blog and FAQ in Phase 1. Reddit and YouTube added in Phase 2.",                                                                             handoff: "Approve → Distribution. Revise → back to agent. Reject → archived.",                         human: "This IS the human. No bulk approvals.", hardGate: true },
...
export default function AgentSystemTab() {
  ...
  return (
    <div>
      ...
      <p style={{ color: "#a1a1aa", fontSize: 14, marginBottom: 28, maxWidth: 640, lineHeight: 1.65 }}>
        The full agent architecture that closes the visibility gaps. Click any node to see what it owns, its DB interactions, and its human gate.
      </p>

      {/* ── SVG diagram ─────────────────────────────────────────────────── */}
      <div style={{ background: "#0c0c0f", border: "1px solid #1f1f23", borderRadius: 14, padding: "16px 4px 12px", marginBottom: 20, overflowX: "auto" }}>
        <svg width="100%" viewBox="0 0 680 668" role="img" style={{ minWidth: 560, display: "block" }}>
          <defs>
            {/* flow arrow */}
          ...
          <g stroke={FLOW_C} strokeWidth="1" fill="none">
            ...
            <path d="M398 218 L448 248 L573 324" markerEnd="url(#arr)"/>
            <line x1="70" y1="368" x2="168" y2="400" markerEnd="url(#arr)"/>
            <line x1="180" y1="368" x2="200" y2="400" markerEnd="url(#arr)"/>
            <path d="M638 346 L648 346 L648 568 L630 568" strokeDasharray="4 3" markerEnd="url(#arr)"/>
          ...
          <g stroke={FLOW_C} strokeWidth="1" fill="none" opacity={0.3}>
            ...
            <path d="M308 444 Q380 460 440 472" markerEnd="url(#arr)"/>
            <path d="M472 516 Q410 546 366 568" strokeDasharray="4 3" markerEnd="url(#arr)"/>
          ...
          {N("schema", 508, 324, 130, 44, "amber",  "Schema agent",         "JSON-LD + llms.txt")}
          ...
          {/* Pipeline signals — dashed border (green but dashed) */}
          ...
          {N("outreach", 368, 472, 198, 44, "pink",   "Roundup outreach agent", "Get Serval into roundups", true)}
        </svg>
      </div>

      {/* ── Detail panel ───────────────────────────────────────────────── */}
      <div style={{ background: "#111113", border: "1px solid #1f1f23", borderRadius: 12, padding: "18px 22px", minHeight: 88, transition: "all 0.2s ease" }}>
        {!detail ? (
          <p style={{ color: "#3f3f46", fontSize: 13, textAlign: "center", margin: "14px 0", fontStyle: "italic" }}>
            Click any node to see what it owns, its DB interactions, and its human gate.
          </p>
        ) : (
          <div>
            {/* Header */}
            <div style={{ display: "flex", alignItems: "baseline", gap: 10, flexWrap: "wrap", marginBottom: 14 }}>
              <span style={{ color: "#fafafa", fontSize: 15, fontWeight: 700 }}>{detail.name}</span>
              <span style={{
              ...
              <span style={{ fontSize: 13, color: "#71717a", lineHeight: 1.5 }}>{detail.role}</span>
            </div>

            {/* Fields */}
            <div style={{ display: "grid", gap: "10px 0" }}>
              {[
                ...
                <div key={label} style={{ display: "grid", gridTemplateColumns: "100px 1fr", gap: "0 12px" }}>
                  <span style={{ color: "#52525b", fontSize: 11, fontWeight: 600, paddingTop: 2, textTransform: "uppercase", letterSpacing: "0.08em" }}>{label}</span>
                  <span style={{ color, fontSize: 13, lineHeight: 1.6, fontWeight: detail.hardGate && label === "Human role" ? 600 : 400 }}>{value}</span>
                </div>
              ))}
            </div>
          </div>
